  ...
  const addItemToCartHandler = (item) => {
    ...
    items.forEach((element, index) => {
      ...
      if (element.id === item.id) {
        ...
        axios
          .put(
            `https://crudcrud.com/api/a3240a552a12475abbb802a23eccc6f0/cart${emailId}/${arr[index]._id}`,
            updatedData
          )
          .then((res) => {
    ...
    if (flag === false) {
      axios
        .post(
          `https://crudcrud.com/api/a3240a552a12475abbb802a23eccc6f0/cart${emailId}`,
          { ...item, quantity: 1 }
        )
        .then((res) => {
          arr.push(res.data);
